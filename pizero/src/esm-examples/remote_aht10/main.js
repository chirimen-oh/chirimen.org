// ===================================================
// AHT10 温湿度センサーの値を WebSocket で送信するプログラム
// ===================================================

// --- ライブラリの読み込み ---
// I2C通信（センサーとのデータのやりとり）に使うライブラリ
import { requestI2CAccess } from "node-web-i2c";
// AHT10（温度と湿度を測れるセンサー）を動かすためのライブラリ
...
// AHT10センサーを初期化（使えるようにする）
const aht10 = new AHT10(i2cPort, 0x38);
await aht10.init();
console.log("AHT10センサーの準備ができました");

// --- WebSocketリレーの準備 ---
// リレーサーバーに接続
const relay = RelayServer("chirimentest", "chirimenSocket");
// データを送れる「チャンネル」を作る
const channel = await relay.subscribe("chirimenAHT10");
console.log("WebSocketリレーサービスに接続しました");

// --- センサーからデータを読み取る関数 ---
async function readSensorData() {
  const { temperature, humidity } = await aht10.readData();
  console.log(
