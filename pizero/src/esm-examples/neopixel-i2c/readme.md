...
```sh
npm i node-web-i2c @chirimen/neopixel-i2c
```
...
Note: 最小限の使い方は `npix.setGlobal(r, g, b)` で全 LED を一括で同じ色に点灯させるものです。main.js では加えて `setPixels()` による LED の個別制御・パターン表示のデモを行っています。
