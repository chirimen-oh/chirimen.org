...
> Note: 非同期処理の関数の呼び出しに関してはそれぞれ`async` を付け、「3. 26番ポートを「出力モード」に設定」が終わってから、「4. Web I2C API の初期化」を開始するように書いてあることを確認しましょう。
...
> Note: ここでも同様に非同期処理の関数の呼び出しに関してはそれぞれ`async` を忘れずに。
