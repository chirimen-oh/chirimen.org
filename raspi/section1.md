...
ここでは、**MOSFET** を使って ギアモータ（ちびギアモータ）の単純な ON/OFF を制御してみましょう。
  ...
  >Note2: Raspberry PiのGPIOポートは、流せる電流の上限が決められています。([合計で50mA](https://elinux.org/RPi_Low-level_peripherals#Power_pins)) また電圧も3.3Vに限定されています。小さなLED数個の場合はこの条件内で使えますが、モーターやソレノイド、パワーLEDなどは直結できません。このようなケースに使えます。
