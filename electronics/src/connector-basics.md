...
- **4本の接点**：すべてのUSBコネクタには少なくとも4本の接点がある（5本のものや、[USB 3.0以降](http://en.wikipedia.org/wiki/USB_3.0)ではさらに多くの接点を持つものもある）。これらは電源、グラウンド、2本のデータ線（D+とD-）用である。USBコネクタは5V、最大500mAを伝送するように設計されている。
...
たとえばUSBが登場するはるか以前、[テキサス・インスツルメンツのグラフ電卓](http://en.wikipedia.org/wiki/TI-85)は、シリアル通信用のコネクタとして2.5mmのTRSを使っていた。
...
厳密には[IEC 60320-1](http://en.wikipedia.org/wiki/Iec_connector)のC13（メス）およびC14（オス）コネクタである。
...
[SMAコネクタ](http://en.wikipedia.org/wiki/SMA_connector)の紛らわしい命名規則についても簡単に触れておく。
