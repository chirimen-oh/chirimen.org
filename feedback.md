...
手元の PC でサイトをビルドしながら編集したい場合の最小手順です。**コントリビューションの全体の流れ（事前準備・フォークから PR まで）は [CONTRIBUTING](/CONTRIBUTING) を参照してください。**

OS に応じた手順で ruby をインストールし `gem install bundler` で bundler もインストールしてください (一般的な Ruby 開発環境構築と同じ)。ruby と bundler まで用意できていたら次の手順でサイトをビルドできます:
