...
ここで、await 接頭詞を付けることができる非同期関数とは、先述の async 接頭詞を付けて自分て定義した関数、およびあらかじめライブラリや組み込み関数として用意されている、Primise 型の帰り値を持った関数です。(Promise 型の詳しい説明はひとまずここでは省きます。そういう型があるということだけ気に留めておいてください。）
