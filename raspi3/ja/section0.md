...
また、async 関数の使い方を知らない人は非同期処理を簡単に書ける [async 関数の使い方の解説ページ](appendix0) をご覧ください。CHIRIMEN Raspi3 のサンプルコードは async 関数を使うことで (Promise を扱ったり古典的なコールバック処理を書くよりも) シンプルで読みやすいコードになっています。
